...
def calculate_bill(units, current_bill_date):
    ...
    if units > 0:
        ...
        if units <= 100:
            ...
            slab_details.append(("0-100", units, 3.96, cost))

        else:
            cost = 100 * 3.96
            total_energy_charge += cost
            slab_details.append(("0-100", 100, 3.96, cost))

            if units <= 300:
                cost = (units - 100) * 10.80
                total_energy_charge += cost
                slab_details.append(("101-300", units - 100, 10.80, cost))

            else:
                cost = 200 * 10.80
                total_energy_charge += cost
                slab_details.append(("101-300", 200, 10.80, cost))

                if units <= 500:
                    cost = (units - 300) * 15.03
                    total_energy_charge += cost
                    slab_details.append(("301-500", units - 300, 15.03, cost))

                else:
                    cost = 200 * 15.03
                    total_energy_charge += cost
                    slab_details.append(("301-500", 200, 15.03, cost))

                    remaining_units = units - 500
                    cost = remaining_units * 17.53
                    total_energy_charge += cost
                    slab_details.append(("500+", remaining_units, 17.53, cost))
